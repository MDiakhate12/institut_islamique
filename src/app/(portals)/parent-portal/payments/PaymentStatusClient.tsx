...
function StatusRow({ label, status }: { label: string; status: 'paid' | 'pending' | 'unpaid' }) {
  return (
    <div className="flex items-center justify-between py-1.5">
      <span className="text-sm">{label}</span>
      {status === 'paid' && (
        <span className="flex items-center gap-1 text-sm font-medium text-green-600">Payé <Check className="h-4 w-4" /></span>
      )}
      {status === 'pending' && (
          ...
          {children.map(c => (
            <div key={c.studentId} className="bg-white rounded-xl border p-4">
              ...
              <div className="divide-y">
                <StatusRow label={PERIOD_LABELS.t1} status={c.t1} />
                <StatusRow label={PERIOD_LABELS.t2} status={c.t2} />
                <StatusRow label={PERIOD_LABELS.t3} status={c.t3} />
              </div>
            </div>
          ))}
        </div>
