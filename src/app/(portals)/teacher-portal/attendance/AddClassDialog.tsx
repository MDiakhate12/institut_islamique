...
import { useAttendanceClassOptions, useAddPinnedAttendanceClass } from '@/modules/attendance/attendance.hooks'
...
export default function AddClassDialog({ open, onClose }: Props) {
  ...
  const { data: options = [] } = useAttendanceClassOptions()
  const addPinned = useAddPinnedAttendanceClass()
  ...
  async function handleAdd() {
    if (!classId) { toast.error('Veuillez sélectionner une classe'); return }
      ...
      <DialogContent showCloseButton={false} className="w-[480px] max-w-[95vw] p-0">
        ...
        <div className="px-6 py-5 space-y-4">
          <p className="text-sm text-gray-500">
            Sélectionnez une classe à épingler dans votre vue des présences.
          </p>

          <div className="space-y-1.5">
            <Label className="text-sm font-medium text-gray-700">Type de matière</Label>
            <Select
              value={subjectCode}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
          <Button variant="outline" onClick={() => { reset(); onClose() }} disabled={addPinned.isPending}>
            Annuler
          </Button>
          <Button
            onClick={handleAdd}
